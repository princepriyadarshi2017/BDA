...
FILE_PATH = os.path.join(os.getcwd(),"restaurant_orders.csv")
    ...
    try:
        for order in get_order_instance(file_path=FILE_PATH):
            ...
            producer.produce(topic=topic,
                            key=string_serializer(str(uuid4()), order_to_dict), #str(uuid4()) change to a "key" so it will always go to same partition
                            value=json_serializer(order, SerializationContext(topic, MessageField.VALUE)),
                            on_delivery=delivery_report)
            break #remove to produce all data 
